...
export default function ApplyPage({ params }) {
  ...
  async function onSubmit(values) {
    try {
      ...
      const data = await api("/candidates/upload", { method: "POST", body: formData });
      setResult(data);
    } catch (error) {
      ...
      <Panel className="w-full">
        ...
        {result ? (
          <div className="mt-5 grid gap-2">
            <p className="font-medium text-success">Application submitted and AI workflow started.</p>
            <p className="text-sm text-muted">Workflow ID: {result.workflow._id}</p>
          </div>
        ) : (
          <form className="mt-5 grid gap-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <Field label="Resume PDF"><Input type="file" accept="application/pdf" {...register("resume", { required: true })} /></Field>
