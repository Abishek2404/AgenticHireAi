...
const agentHandlers = {
  ...
  matching_agent: async (context) => {
    const ragContext = await retrieveHiringContext(context.job.title);
    const matchResult = await runMatchingAgent({
      ...context,
      parsedResume: context.workflow.state.parsedResume,
      ragContext
    });
    context.workflow.state.matchResult = matchResult;
    await Candidate.findByIdAndUpdate(context.candidate._id, { match_score: matchResult.data.match_score });
    return matchResult;
  },
